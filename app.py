...
def get_most_used_values():
    ...
    if DB_TYPE == "postgres":
        with engine.connect() as conn:
            ...
            data['precos_cores'] = [{'id': p[0], 'cor': fix_text(p[1]).upper(), 'preco_kg': p[2]} for p in res_precos.fetchall()]
    else:
        conn = sqlite3.connect(DB_NAME)
        ...
        data['precos_cores'] = [{'id': p['id'], 'cor': fix_text(p['cor']).upper(), 'preco_kg': p['preco_kg']} for p in precos_bruto]
        conn.close()
    return data
        ...
        with engine.connect() as conn:
            ...
            for r in res_reg.fetchall():
                ...
                r_dict = {
                    ...
                    'codigo': fix_text(r[2]).upper(),
                    'descricao': fix_text(r[3]).upper(),
                    'fornecedor': fix_text(r[4]).upper(),
                    'linha': fix_text(r[5]).upper(),
                    'cor': fix_text(r[6]).upper(),
                    'medida': fix_text(r[7]).upper(),
                    'sistema': fix_text(r[8]).upper(),
                    'valor': round(val_calc, 2),
                    'observacao': fix_text(r[10]).upper(),
                    'estoque': r[11] if r[11] is not None else 0,
                    'nescessario': r[12] if r[12] is not None else 0,
                    'material': fix_text(r[13]).upper(),
...
def salvar():
    ...
    linha = fix_text(request.form.get('linha')).upper()
    cor = fix_text(request.form.get('cor')).strip().upper()
    medida = fix_text(request.form.get('medida')).upper()
    sistema = fix_text(request.form.get('sistema')).upper()
    observacao = fix_text(request.form.get('observacao')).upper()
...
def exportar_carrinho_pdf():
    ...
    for item in carrinho:
        codigo = fix_text(item.get('codigo', '')).upper()
        ...
        data.append([img_element, Paragraph(codigo, style_cell), Paragraph(fix_text(item.get('descricao', '')).upper(), style_desc), Paragraph(fix_text(item.get('cor', '')).upper(), style_cell), Paragraph(str(item.get('quantidade', 0)), style_cell)])
    ...
    if DB_TYPE == "postgres":
        with engine.begin() as conn:
            ...
            if row:
                ...
                conn.execute(text('''
                    INSERT INTO acessorios (imagem, codigo, descricao, fornecedor, linha, cor, medida, sistema, valor, observacao, estoque, nescessario, material, peso_kg_m) 
                    VALUES (:img, :codigo, :descricao, :fornecedor, :linha, :cor, :medida, :sistema, :valor, :observacao, :estoque, :nescessario, :material, :peso_kg_m)
                '''), {"img": img, "codigo": novo_codigo.upper(), "descricao": row[2], "fornecedor": row[3], "linha": row[4], "cor": row[5], "medida": row[6], "sistema": row[7], "valor": row[8], "observacao": row[9], "estoque": row[10], "nescessario": row[11], "material": row[12], "peso_kg_m": row[13]})
    else:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        ...
        if row:
            ...
            cursor.execute('''
                INSERT INTO acessorios (imagem, codigo, descricao, fornecedor, linha, cor, medida, sistema, valor, observacao, estoque, nescessario, material, peso_kg_m) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ''', (img, novo_codigo.upper(), row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13]))
            conn.commit()
            conn.close()
    return redirect(url_for('index'))
...
def importar_excel():
    ...
    try:
        ...
        for _, row in df.iterrows():
            codigo = fix_text(row.get('codigo', '')).strip().upper()
            ...
            medida = fix_text(row.get('medida', '')).upper()
            sistema = fix_text(row.get('sistema', '')).upper()
            observacao = fix_text(row.get('observacao', '')).upper()
            material = fix_text(row.get('material', '')).strip().upper()
            
            img_csv = fix_text(str(row.get('imagem', row.get('image', '')))).strip()
            if img_csv.lower() == 'nan': 
                img_csv = ""
            ...
            if not img_final and img_csv: 
                img_final = os.path.basename(img_csv)
    ...
    for row in rows:
        ...
        for c in valid_cols:
            ...
            if idx == 0:
                ...
                raw_val = row[idx]
                txt = fix_text(raw_val).upper() if raw_val is not None and not pd.isna(raw_val) and str(raw_val).lower() != 'nan' else ""
